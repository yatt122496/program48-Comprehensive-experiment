		...
		if(system_task_return)break;//需要返回
 		if(second!=calendar.sec) {	//秒钟改变了
			...
			if(t%2==0) {//等待2秒钟
				...
				}else POINT_COLOR=GREEN;	//正常为棕红色字体显示
				gui_show_num(xoff+90,OTHER_TOPY,2,POINT_COLOR,60,temperate/10,0X80);	//XX
				gui_show_ptchar(xoff+150,OTHER_TOPY,lcddev.width,lcddev.height,0,POINT_COLOR,60,'.',0);	//"."
				gui_show_ptchar(xoff+180-15,OTHER_TOPY,lcddev.width,lcddev.height,0,POINT_COLOR,60,temperate%10+'0',0);//显示小数
				gui_show_ptchar(xoff+210-10,OTHER_TOPY,lcddev.width,lcddev.height,0,POINT_COLOR,60,95+' ',0);//显示℃
				if(t>0)t=0;
			}
